...
def vectorize(ex, model):
    ...
    vectorized_ex['position_rep'] = None
    ...
    if code.struc is not None:
        ...
        vectorized_ex['position_rep'] = torch.LongTensor(code.position_rep)
...
def batchify(batch):
    ...
    position = [ex['position_rep'] for ex in batch]
    ...
    position_rep = torch.zeros(batch_size, 512, 128, dtype=torch.long) \
        if use_code_struc else None
    ...
    for i in range(batch_size):
        ...
        if use_code_struc:
            ...
            position_rep[i, :, :].copy_(position[i][:512, :128])
        #
